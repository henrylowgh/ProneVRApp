Shader "Custom/Atmosphere" 
{
	SubShader 
	{
    	...
    	Pass 
    	{
			...
			uniform float bleeding;			// Bleeding of the outer atmosphere along the night face of Earth
			...
			half4 frag(v2f IN) : COLOR
			{
				...
				float fCameraHeight = length(v3CameraPos)*1.03;					// The camera's current height
				float fCameraHeight2 = fCameraHeight*fCameraHeight;			// fCameraHeight^2
			
				// Get the ray from the camera to the vertex and its length (which is the far point of the ray passing through the atmosphere)
				float3 v3Pos = mul(unity_ObjectToWorld, IN.opos).xyz - v3Translate;
				float3 v3Ray = v3Pos - v3CameraPos;
				float fFar = length(v3Ray);
				v3Ray /= fFar;
				
				// Calculate the closest intersection of the ray with the outer atmosphere (which is the near point of the ray passing through the atmosphere)
				float B = 2 * dot(v3CameraPos, v3Ray);
				...
				{
					float fHeight = length(v3SamplePoint)*0.98;
				...
				float fCos = dot(v3LightPos, (v3CameraPos - v3Pos)) / length(v3CameraPos - v3Pos);
				float fCos2 = fCos*fCos;
				float3 col = getRayleighPhase(fCos2) * (v3FrontColor * (v3InvWavelength * fKrESun)) + getMiePhase(fCos, fCos2, g, g2) * (v3FrontColor * fKmESun) ;
